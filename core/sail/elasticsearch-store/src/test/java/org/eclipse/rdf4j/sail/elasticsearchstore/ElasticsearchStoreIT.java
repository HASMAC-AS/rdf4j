...
public class ElasticsearchStoreIT extends ElasticsearchTestContainer {

	private static final Logger logger = LoggerFactory.getLogger(ElasticsearchStoreIT.class);

	@Test
	public void testInstantiate() {
		ElasticsearchStore elasticsearchStore = new ElasticsearchStore(hostname,
				transportPort, clusterName, "testindex");
	...
	public void testSailRepository() {
		SailRepository elasticsearchStore = new SailRepository(
				new ElasticsearchStore(hostname, transportPort, clusterName, "testindex"));
		elasticsearchStore.shutDown();
	}
	...
	public void testShutdownAndRecreate() {
		...
		elasticsearchStore.shutDown();
		elasticsearchStore = new ElasticsearchStore(hostname, transportPort, clusterName,
				"testindex");
		try (NotifyingSailConnection connection = elasticsearchStore.getConnection()) {
			connection.begin(IsolationLevels.NONE);
			connection.addStatement(RDF.TYPE, RDF.TYPE, RDFS.RESOURCE);
	...
	private ClientProvider initElasticsearchStoreForGcTest() {
		ElasticsearchStore sail = new ElasticsearchStore(hostname, transportPort, clusterName,
				"testindex");
