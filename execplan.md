...
- [x] (2026-01-17 13:10Z) Add failing test for query filtering.
- [x] (2026-01-17 13:10Z) Implement batched reindex using query results.
- [x] (2026-01-17 13:10Z) Update tests, formatting, and targeted verification.
- [ ] (2026-01-17 13:10Z) Summarize outcomes and finalize artifacts.
...
- Decision: Reuse `reindexQuery` without adding `ORDER BY` and batch-sort query results in code.
  Rationale: Preserves user-configured query filters while avoiding a global sort requirement.
  Date/Author: 2026-01-17 / Codex

## Outcomes & Retrospective

The reindex path now batches statements from the configured reindex query, sorts each batch, and indexes using the existing bulk API. Dedicated tests exercise unsorted statement iteration and confirm that the reindex query is honored. The reindex process no longer requires a global ordered query, reducing memory pressure for large stores while keeping query filtering intact. The remaining work is to finalize summary artifacts.

## Context and Orientation

Lucene reindexing logic lives in `core/sail/lucene-api/src/main/java/org/eclipse/rdf4j/sail/lucene/LuceneSail.java` in the `reindex()` method. It previously used a SPARQL query with `ORDER BY ?s` and looped over results, assuming subject ordering to batch statements per subject before calling `luceneIndex.addDocuments`. Tests for Lucene reindexing live under `core/sail/lucene/src/test/java/org/eclipse/rdf4j/sail/lucene/`, including the `ReindexBatchingTest`.

The new reindex loop uses the configured reindex query (without appending `ORDER BY`) to iterate over statements, collects a bounded list of statements, sorts that list, and passes the batch to the existing bulk indexing API.
...
Next, change `LuceneSail.reindex()` to iterate using the configured reindex query without appending `ORDER BY`. Collect a fixed number of statements into a list, sort the list by subject (and optionally predicate/object/context to ensure stable ordering), and feed that list to the existing lucene indexing batching logic. Repeat until the iterator is exhausted. Ensure each batch is indexed correctly and any remaining statements are handled at the end.
...
Acceptance requires: the new test fails before the reindex change and passes after; lucene module tests pass; and the reindex logic now iterates over the configured query in bounded batches, sorts each batch, and indexes using the existing lucene index calls. The new tests should demonstrate that unsorted inputs are properly handled by the reindexing path and that the query filter is honored.
...
Plan updated 2026-01-17: recorded the query-filtering test, the decision to preserve user queries without ORDER BY, and updated references to the query-based batching flow.
