...
The Boot module now imports the legacy XML configurations, serves both server and workbench endpoints from embedded Tomcat, and ships with an integration test that guards the critical `/rdf4j-server` and `/rdf4j-workbench` flows. The e2e harness builds and launches the Boot jar directly, avoiding Docker while preserving cleanup semantics. Formatters were run to normalize indentation and XML layout, and focused module tests continue to pass. Remaining known gap: Playwright cannot download Chromium in this environment (HTTP 403), so full browser automation still depends on external network allowances.
